...
export class SlackApiService extends AuthedApi {
    ...
    public getTeams(): Promise<FE_SlackTeam[]> {
        return this.authedGet("/_dimension/api/v1/dimension/slack/teams")
            .pipe(map(r => r.json())).toPromise();
    }

    public getChannels(teamId: string): Promise<FE_SlackChannel[]> {
        return this.authedGet("/_dimension/api/v1/dimension/slack/teams/" + teamId + "/channels")
            .pipe(map(r => r.json())).toPromise();
    }

    public getAuthUrl(): Promise<string> {
        return this.authedGet("/_dimension/api/v1/dimension/slack/auth")
            .pipe(map(r => r.json())).toPromise()
            .then(r => r["authUrl"]);
    }
}
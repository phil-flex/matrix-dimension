...
export class StickerApiService extends AuthedApi {
    ...
    public getConfig(): Promise<FE_StickerConfig> {
        return this.authedGet("/_dimension/api/v1/dimension/stickers/config")
            .pipe(map(r => r.json())).toPromise();
    }

    public getPacks(): Promise<FE_UserStickerPack[]> {
        return this.authedGet("/_dimension/api/v1/dimension/stickers/packs")
            .pipe(map(r => r.json())).toPromise();
    }

    public togglePackSelection(packId: number, isSelected: boolean): Promise<any> {
        return this.authedPost("/_dimension/api/v1/dimension/stickers/packs/" + packId + "/selected", {isSelected: isSelected})
            .pipe(map(r => r.json())).toPromise();
    }

    public importStickerpack(packUrl: string): Promise<FE_UserStickerPack> {
        return this.authedPost("/_dimension/api/v1/dimension/stickers/packs/import", {packUrl})
            .pipe(map(r => r.json())).toPromise();
    }
}

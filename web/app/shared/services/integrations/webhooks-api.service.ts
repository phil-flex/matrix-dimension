...
export class WebhooksApiService extends AuthedApi {
    ...
    public createWebhook(roomId: string, options: FE_WebhookOptions): Promise<FE_Webhook> {
        return this.authedPost<FE_Webhook>("/_dimension/api/v1/dimension/webhooks/room/" + roomId + "/webhooks/new", options).toPromise();
    }

    public updateWebhook(roomId: string, hookId: string, options: FE_WebhookOptions): Promise<FE_Webhook> {
        return this.authedPost<FE_Webhook>("/_dimension/api/v1/dimension/webhooks/room/" + roomId + "/webhooks/" + hookId, options).toPromise();
    }

    public deleteWebhook(roomId: string, hookId: string): Promise<any> {
        return this.authedDelete("/_dimension/api/v1/dimension/webhooks/room/" + roomId + "/webhooks/" + hookId).toPromise();
    }
}
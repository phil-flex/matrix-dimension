...
export class AdminApiService extends AuthedApi {
    ...
    public isAdmin(): Promise<any> {
        return this.authedGet("/_dimension/api/v1/dimension/admin/check")
            .pipe(map(r => r.json())).toPromise();
    }

    public getConfig(): Promise<FE_DimensionConfig> {
        return this.authedGet("/_dimension/api/v1/dimension/admin/config")
            .pipe(map(r => r.json())).toPromise();
    }

    public getVersion(): Promise<FE_DimensionVersion> {
        return this.authedGet("/_dimension/api/v1/dimension/admin/version")
            .pipe(map(r => r.json())).toPromise();
    }

    public logoutAll(): Promise<any> {
        return this.authedPost("/_dimension/api/v1/dimension/admin/sessions/logout/all")
            .pipe(map(r => r.json())).toPromise();
    }
}

...
export class AdminIrcApiService extends AuthedApi {
    ...
    public getBridges(): Promise<FE_IrcBridge[]> {
        return this.authedGet("/_dimension/api/v1/dimension/admin/irc/all")
            .pipe(map(r => r.json())).toPromise();
    }

    public getBridge(bridgeId: number): Promise<FE_IrcBridge> {
        return this.authedGet("/_dimension/api/v1/dimension/admin/irc/" + bridgeId)
            .pipe(map(r => r.json())).toPromise();
    }

    public newFromUpstream(upstream: FE_Upstream): Promise<FE_IrcBridge> {
        return this.authedPost("/_dimension/api/v1/dimension/admin/irc/new/upstream", {upstreamId: upstream.id})
            .pipe(map(r => r.json())).toPromise();
    }

    public newSelfhosted(provisionUrl: string): Promise<FE_IrcBridge> {
        return this.authedPost("/_dimension/api/v1/dimension/admin/irc/new/selfhosted", {provisionUrl: provisionUrl})
            .pipe(map(r => r.json())).toPromise();
    }

...
export class AdminTelegramApiService extends AuthedApi {
    ...
    public getBridges(): Promise<FE_TelegramBridge[]> {
        return this.authedGet("/_dimension/api/v1/dimension/admin/telegram/all").map(r => r.json()).toPromise();
    }

    public getBridge(bridgeId: number): Promise<FE_TelegramBridge> {
        return this.authedGet("/_dimension/api/v1/dimension/admin/telegram/" + bridgeId).map(r => r.json()).toPromise();
    }

    public newFromUpstream(upstream: FE_Upstream): Promise<FE_TelegramBridge> {
        return this.authedPost("/_dimension/api/v1/dimension/admin/telegram/new/upstream", {upstreamId: upstream.id}).map(r => r.json()).toPromise();
    }

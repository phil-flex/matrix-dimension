...
export class AdminUpstreamApiService extends AuthedApi {
    ...
    public getUpstreams(): Promise<FE_Upstream[]> {
        return this.authedGet<FE_Upstream[]>("/_dimension/api/v1/dimension/admin/upstreams/all").toPromise();
    }

    public newUpstream(name: string, type: string, scalarUrl: string, apiUrl: string): Promise<FE_Upstream> {
        return this.authedPost<FE_Upstream>("/_dimension/api/v1/dimension/admin/upstreams/new", {
            name: name,
            type: type,
            scalarUrl: scalarUrl,
            apiUrl: apiUrl,

...
export class AdminNebApiService extends AuthedApi {
    ...
    public getConfigurations(): Promise<FE_NebConfiguration[]> {
        return this.authedGet("/_dimension/api/v1/dimension/admin/neb/all")
            .pipe(map(r => r.json())).toPromise();
    }

    public getConfiguration(nebId: number): Promise<FE_NebConfiguration> {
        return this.authedGet("/_dimension/api/v1/dimension/admin/neb/" + nebId + "/config")
            .pipe(map(r => r.json())).toPromise();
    }

    public newUpstreamConfiguration(upstream: FE_Upstream): Promise<FE_NebConfiguration> {
        return this.authedPost("/_dimension/api/v1/dimension/admin/neb/new/upstream", {upstreamId: upstream.id})
            .pipe(map(r => r.json())).toPromise();
    }
    ...
    public toggleIntegration(nebId: number, integrationType: string, setEnabled: boolean): Promise<any> {
        return this.authedPost("/_dimension/api/v1/dimension/admin/neb/" + nebId + "/integration/" + integrationType + "/enabled", {enabled: setEnabled})
            .pipe(map(r => r.json())).toPromise();
    }

    public setIntegrationConfiguration(nebId: number, integrationType: string, configuration: any): Promise<any> {
        return this.authedPost("/_dimension/api/v1/dimension/admin/neb/" + nebId + "/integration/" + integrationType + "/config", configuration)
            .pipe(map(r => r.json())).toPromise();
    }

    public getIntegrationConfiguration(nebId: number, integrationType: string): Promise<any> {
        return this.authedGet("/_dimension/api/v1/dimension/admin/neb/" + nebId + "/integration/" + integrationType + "/config")
            .pipe(map(r => r.json())).toPromise();
    }
}

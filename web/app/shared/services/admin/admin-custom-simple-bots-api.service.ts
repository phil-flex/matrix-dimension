...
export class AdminCustomSimpleBotsApiService extends AuthedApi {
    ...
    public getBots(): Promise<FE_CustomSimpleBot[]> {
        return this.authedGet("/_dimension/api/v1/dimension/admin/bots/simple/custom/all")
            .pipe(map(r => r.json())).toPromise();
    }

    public getBot(id: number): Promise<FE_CustomSimpleBot> {
        return this.authedGet("/_dimension/api/v1/dimension/admin/bots/simple/custom/" + id)
            .pipe(map(r => r.json())).toPromise();
    }

    public updateBot(id: number, config: FE_CustomSimpleBotTemplate): Promise<FE_CustomSimpleBot> {
        return this.authedPost("/_dimension/api/v1/dimension/admin/bots/simple/custom/" + id, config)
            .pipe(map(r => r.json())).toPromise();
    }

    public createBot(config: FE_CustomSimpleBotTemplate): Promise<FE_CustomSimpleBot> {
        return this.authedPost("/_dimension/api/v1/dimension/admin/bots/simple/custom/new", config)
            .pipe(map(r => r.json())).toPromise();
    }

    public getProfile(userId: string): Promise<FE_UserProfile> {
        return this.authedGet("/_dimension/api/v1/dimension/admin/bots/simple/custom/profile/" + userId)
            .pipe(map(r => r.json())).toPromise();
    }
}

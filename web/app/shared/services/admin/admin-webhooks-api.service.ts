...
export class AdminWebhooksApiService extends AuthedApi {
    ...
    public getBridges(): Promise<FE_WebhooksBridge[]> {
        return this.authedGet("/_dimension/api/v1/dimension/admin/webhooks/all").map(r => r.json()).toPromise();
    }

    public getBridge(bridgeId: number): Promise<FE_WebhooksBridge> {
        return this.authedGet("/_dimension/api/v1/dimension/admin/webhooks/" + bridgeId).map(r => r.json()).toPromise();
    }

    public newFromUpstream(upstream: FE_Upstream): Promise<FE_WebhooksBridge> {
        return this.authedPost("/_dimension/api/v1/dimension/admin/webhooks/new/upstream", {upstreamId: upstream.id}).map(r => r.json()).toPromise();
    }

    public newSelfhosted(provisionUrl: string, sharedSecret: string): Promise<FE_WebhooksBridge> {
        return this.authedPost("/_dimension/api/v1/dimension/admin/webhooks/new/selfhosted", {
    ...
    public updateSelfhosted(bridgeId: number, provisionUrl: string, sharedSecret: string): Promise<FE_WebhooksBridge> {
        return this.authedPost("/_dimension/api/v1/dimension/admin/webhooks/" + bridgeId, {
            provisionUrl: provisionUrl,
            sharedSecret: sharedSecret,
        }).map(r => r.json()).toPromise();
    }

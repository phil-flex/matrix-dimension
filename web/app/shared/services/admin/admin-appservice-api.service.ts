...
export class AdminAppserviceApiService extends AuthedApi {
    ...
    public getAppservices(): Promise<FE_Appservice[]> {
        return this.authedGet("/_dimension/api/v1/dimension/admin/appservices/all")
            .pipe(map(r => r.json())).toPromise();
    }

    public getAppservice(appserviceId: string): Promise<FE_Appservice> {
        return this.authedGet("/_dimension/api/v1/dimension/admin/appservices/" + appserviceId)
            .pipe(map(r => r.json())).toPromise();
    }

    public createAppservice(userPrefix: string): Promise<FE_Appservice> {
        return this.authedPost("/_dimension/api/v1/dimension/admin/appservices/new", {userPrefix: userPrefix})
            .pipe(map(r => r.json())).toPromise();
    }

    public test(appserviceId: string): Promise<any> {
        return this.authedPost("/_dimension/api/v1/dimension/admin/appservices/" + appserviceId + "/test")
            .pipe(map(r => r.json())).toPromise();
    }
}

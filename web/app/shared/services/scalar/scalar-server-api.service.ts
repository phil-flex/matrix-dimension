...
export class ScalarServerApiService extends AuthedApi {
    ...
    public ping(): Promise<any> {
        return this.http.get("/_dimension/api/v1/scalar/ping").toPromise();
    }

    public getAccount(): Promise<FE_ScalarAccountResponse> {
        return this.authedGet<FE_ScalarAccountResponse>("/_dimension/api/v1/scalar/account", {v: SCALAR_API_VERSION}).toPromise();
    }

    public register(openId: FE_ScalarOpenIdRequestBody): Promise<FE_ScalarRegisterResponse> {
        return this.http.post<FE_ScalarRegisterResponse>("/_dimension/api/v1/scalar/register", openId, {
